...
public final class SceneJsonCodec {
    ...
    public static String encode(SceneSnapshot snapshot) {
        SceneFileV1 file = toDto(snapshot);
        return GSON.toJson(file);
    }

    /** Parse JSON into a {@link SceneSnapshot} (authoring fields + clusters). */
    ...
    static SceneFileV1 toDto(SceneSnapshot snapshot) {
        ...
        for (ClusterSite site : snapshot.clusters()) {
            ...
            Rgba c = site.color();
            // RGBA components are 0..1 in the file.
    ...
    static SceneSnapshot fromDto(SceneFileV1 dto) throws SceneJsonException {
